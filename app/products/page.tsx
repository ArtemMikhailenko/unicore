...
function ProductCard({ id, title, description, imageSrc, logo, className = '' }: ProductCardProps) {
  return (
    <Link 
      href={`/payment/${id}`}
      className={`relative bg-[#0A0218] rounded-[30px] p-[30px] 2xl:p-[40px] overflow-hidden cursor-pointer hover:scale-[1.02] transition-transform ${className}`}
      style={{
        ...
        <div className="flex flex-col gap-[10px]">
          <div className="flex items-center gap-3">
            <Image 
              ...
              width={48}
              height={48}
              className="w-12 h-12 2xl:w-16 2xl:h-16 rounded-lg object-cover"
            />
            <h3 
              className="text-[16px] lg:text-[24px] 2xl:text-[32px] leading-[29px] 2xl:leading-[40px] font-bold text-white"
...
const products = [
  {
    ...
    className: 'w-full h-[200px] lg:h-[280px] 2xl:h-[350px]'
  },
  {
    id: 'wuthering-waves',
...
export default function ProductsPage() {
  return (
    <main className="min-h-screen bg-[#0A0218] flex flex-col items-center px-4 lg:px-[10%] py-[30px] lg:py-[50px] relative overflow-hidden">
      {/* Background Lines */}
      ...
      <div className="w-full max-w-[1132px] lg:max-w-none flex flex-col items-center gap-[25px] relative z-10">
        {/* Title */}
        <h1 
          className="text-[36px] 2xl:text-[48px] leading-[44px] 2xl:leading-[56px] font-bold text-white text-center"
          style={{ fontFamily: 'var(--font-hywenhei, system-ui)' }}
        >
          Products
        </h1>
