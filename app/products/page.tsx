...
function ProductCard({ id, title, description, imageSrc, logo, className = '' }: ProductCardProps) {
  return (
    <Link 
      href={`/payment/${id}`}
      className={`relative bg-[#0A0218] rounded-[30px] 2xl:rounded-[50px] 3xl:rounded-[49px] p-[30px] 2xl:p-[50px] 3xl:p-[49px] overflow-hidden cursor-pointer hover:scale-[1.02] transition-transform ${className}`}
      style={{
    ...
    >
      ...
      {/* Content */}
      <div className="relative z-10 flex flex-col justify-between h-full gap-[10px] 2xl:gap-[20px] 3xl:gap-[21px]">
        <div className="flex flex-col gap-[10px] 2xl:gap-[20px] 3xl:gap-[21px]">
          <div className="flex items-center gap-3 2xl:gap-5 3xl:gap-7">
            ...
            />
            <h3 
              className="text-[16px] lg:text-[24px] 2xl:text-[24px] 3xl:text-[39px] leading-[29px] 2xl:leading-[50px] 3xl:leading-[48px] font-bold text-white"
              style={{ fontFamily: 'var(--font-hywenhei, system-ui)' }}
            >
              {title}
            </h3>
          </div>
          <p className="text-[12px] lg:text-[14px] 2xl:text-[14px] 3xl:text-[22px] leading-[160%] text-[#737AAE]">
            {description}
          </p>
        </div>
      </div>
...
const products = [
  {
    ...
    className: 'w-full h-[200px] lg:h-[280px] 2xl:h-[450px] 3xl:h-[434px]'
  },
  {
    id: 'wuthering-waves',
...
export default function ProductsPage() {
  return (
    <main className="min-h-screen bg-[#0A0218] flex flex-col items-center px-4 lg:px-[20%] py-[30px] lg:py-[50px] 2xl:py-[100px] 3xl:py-[98px] relative overflow-hidden">
      {/* Background Lines */}
      <div className="absolute bottom-0 right-0 w-full h-[898px] 2xl:h-[1400px] 3xl:h-[1260px] opacity-90">
        <Image
      ...
      <div className="w-full max-w-[1132px] lg:max-w-none flex flex-col items-center gap-[25px] 2xl:gap-[50px] 3xl:gap-[49px] relative z-10">
        {/* Title */}
        <h1 
          className="text-[36px] 2xl:text-[60px] 3xl:text-[56px] leading-[44px] 2xl:leading-[72px] 3xl:leading-[67px] font-bold text-white text-center"
          style={{ fontFamily: 'var(--font-hywenhei, system-ui)' }}
        >
          Products
        </h1>
        ...
        {/* Products Grid */}
        <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-[32px] 2xl:gap-[50px] 3xl:gap-[49px]">
